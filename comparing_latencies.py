...
networkLatencies='/vault-home/goelayu/NSDI17/goelayu/with-missing-gc-reg/located-ping-times.txt'
storageLatencies='/vault-home/uluyol/paxosstore-results/sm-100K/'
storageLatencyReadFile = 'store-reads.logs'
storageLatencyWriteFile = 'store-writes.logs'

dcIndexMap = { '0' : 'aws/ap-northeast-1','1' : 'aws/ap-northeast-2','2' : 'aws/ap-south-1','3' : 'aws/ap-southeast-1','4' : 'aws/ap-southeast-2','5' : 'aws/ca-central-1','6' : 'aws/eu-central-1','7' : 'aws/eu-west-1','8' : 'aws/eu-west-2','9' : 'aws/sa-east-1','10' : 'aws/us-east-1','11' : 'aws/us-east-2','12' : 'aws/us-west-1','13' : 'aws/us-west-2','14' : 'az/australiaeast','15' : 'az/australiasoutheast','16' : 'az/brazilsouth','17' : 'az/canadacentral','18' : 'az/canadaeast','19' : 'az/centralindia','20' : 'az/centralus','21' : 'az/eastasia','22' : 'az/eastus','23' : 'az/eastus2','24' : 'az/japaneast','25' : 'az/japanwest','26' : 'az/koreacentral','27' : 'az/koreasouth','28' : 'az/northcentralus','29' : 'az/northeurope','30' : 'az/southcentralus','31' : 'az/southeastasia','32' : 'az/southindia','33' : 'az/uksouth','34' : 'az/ukwest','35' : 'az/westcentralus','36' : 'az/westeurope','37' : 'az/westus','38' : 'az/westus2','39' : 'gc/asia-east1','40' : 'gc/asia-northeast1','41' : 'gc/asia-southeast1','42' : 'gc/europe-west1','43' : 'gc/us-central1','44' : 'gc/us-east1','45' : 'gc/us-west1' }
...
def ReadNetworkLatencies():
	...
	latencies = {}
	latencyFile = open(networkLatencies, 'r')
	# errorCount = 0
	# valueCount = 0
	for line in latencyFile.readlines():
		...
		if latency != "error":
			...
			if dc2 not in latencies[dc1]:
				latencies[dc1][dc2] = []
			latencies[dc1][dc2].append(latency)
			# valueCount += 1
...
def ReadStorageLatencies():
	...
	readLatency = {}
	writeLatency = {}
	datacenters=listdir(storageLatencies)
	for dc in datacenters:
		readStoragefile = open(storageLatencies + dc + "/store-reads.log",'r')
		for line in readStoragefile.readlines():
			...
			if status == "success":
				...
				if dcKey not in readLatency:
					readLatency[dcKey] = []
				readLatency[dcKey].append(latency)

		writeStoragefile = open(storageLatencies + dc + "/store-writes.log",'r')
		for line in writeStoragefile.readlines():
			...
			if status == "success":
				...
				if dcKey not in writeLatency:
					writeLatency[dcKey] = []
				writeLatency[dcKey].append(latency)
...
def GetRequestGenerator(networkLatencies, readStorageLatencies, accessSet, listOfReplicas, readQourumSize):
	randomFrontEnd = random.choice(accessSet)
	print "The random frontend for get request", randomFrontEnd
...
def PutRequestGenerator(networkLatencies, storageLatencies, accessSet, listOfReplicas, readQourumSize, writeQourumSize):
	randomFrontEnd = random.choice(accessSet)
	print "The random frontend for put request", randomFrontEnd
...
def main():
	...
	if qourumSystem == "b":
		...
		for i in range(numberOfRequests):	
			sampleGet = GetRequestGenerator(networkLatencies, storageLatencies[0], accessSet, listOfReplicas, readQourumSize)
			samplePut = PutRequestGenerator(networkLatencies, storageLatencies, accessSet, listOfReplicas, readQourumSize, writeQourumSize)
